<template>
  ...
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 bg-teal-800 p-6 rounded-xl shadow-md shadow-black">
    ...
    <div class="mx-auto lg:mx-0">
      <h2 class="text-3xl font-bold tracking-tight text-white sm:text-4xl text-center">Training and Behavior Forum</h2>
        ...
        <div class="mt-10 grid grid-cols-1 gap-x-8 gap-y-16 sm:grid-cols-2 md:grid-cols-3">
          <article v-for="topic in topics" :key="topic.id" class="flex flex-col bg-white justify-between border rounded-xl p-6 shadow-md shadow-black border-gray-400 pt-10">
            <div class="flex items-center gap-x-4 text-xs">
              ...
              <a href="#" class="relative z-10 rounded-full bg-blue-200 px-3 py-1.5 poppins-bold text-gray-600 hover:bg-gray-100">Training</a>
            </div>
...
import AddTrainingModal from "@/components/ForumItems/AddTrainingModal.vue";
...
methods: {
  ...
  async fetchPosts() {
    try {
      const response = await axios.get('http://localhost:3000/api/forum/posts/3', {
