<template>
  ...
  <div class="max-w-7xl mx-auto p-12 bg-teal-700 shadow-lg rounded-lg"> <!-- Increased padding and rounded corners -->
    <div class="text-center mb-8">
      <button @click="showAddTrainingModal = true"
    ...
    <div class="text-center mb-8 animate-fade-in">
      <h2 class="text-4xl font-bold tracking-tight text-teal-50 sm:text-5xl">Training and Behavior Forum</h2>
      <p class="mt-2 text-lg text-teal-50">Lorem ipsum dolor sit amet consectetur, adipisicing elit. Ipsa illo quaerat
        distinctio.</p>
    </div>
    <div class="mt-10">
      <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3"> <!-- Responsive grid setup -->
        <article v-for="topic in topics" :key="topic.PostID"
          ...
          <div class="flex items-center gap-x-4 text-xs text-gray-500 mb-4">
            ...
            <p
              class="z-50 absolute top-16 left-8 bg-teal-300 text-teal-900 font-semibold px-3 py-1 rounded-lg shadow-md text-sm uppercase tracking-wider">
             Training
            </p>
          </div>
          ...
          <div class="jutify-bottom text-center">
    ...
    <AddTrainingModal v-if="showAddTrainingModal" @close="showAddTrainingModal = false" @submit="addNewTopic" />
  </div>
...
import AddTrainingModal from './AddTrainingModal.vue';
...
const fetchUserDetails = async () => {
  ...
  try {
    const response = await instance.get('/api/forum/posts/3', {
