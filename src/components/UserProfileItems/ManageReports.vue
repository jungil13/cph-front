        ...
        <div class="w-full mx-auto p-5 rounded-lg">
            ...
            <div v-if="reports.length" class="grid gap-4 md:grid-cols-2 lg:grid-cols-4 text-center">
                <div v-for="report in reports" :key="report.ReportID"
                    class="relative animate-fade-in flex flex-col bg-stone-900 p-4 rounded-lg shadow-lg shadow-black transition-transform transform hover:scale-105 border border-black">
                    <div class="relative group w-full h-48 mb-4">
                        <img :src="getImageUrl(report.Photo[0])" alt="Report Picture"
                            class="border w-full h-full object-cover rounded-md mb-4 transition-transform duration-300 group-hover:scale-105">
                        <div v-if="report.Photo.length > 1"
                            class="absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                            <div class="flex space-x-2 overflow-auto">
                                <img v-for="(photo, index) in report.Photo" :key="index" :src="getImageUrl(photo)"
                                    alt="Report Photo" class="w-24 h-24 object-cover rounded-md cursor-pointer"
                                    @click="photoPreview = getImageUrl(photo)">
                            </div>
                        </div>
                    </div>
                    <div class="flex-1 mb-4 font-bold">
                        <h2 class="text-xl font-bold text-red-500 mb-2">
                            <span class="text-white">Report Type: </span> ({{ report.ReportType }})
                        </h2>
                        <p class="text-teal-400 mb-2">
                            <span class="text-white">Status: </span>{{ report.Status }}
                        </p>
            ...
            <div v-if="selectedReport"
                class="fixed inset-0 bg-gray-800 bg-opacity-50 flex items-center justify-center top-12">
...
export default {
    ...
    methods: {
        ...
        getImageUrl(photoPath) {
            return photoPath ? `http://localhost:3000/${photoPath}` : 'https://via.placeholder.com/150';
        }
    }
};
</script>
