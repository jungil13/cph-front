  ...
  <div class="bg-teal-50 flex flex-col md:flex-row items-center justify-center text-center py-12">
    ...
    <div class="flex justify-center mb-8">
      <select
        ...
        class="border border-gray-300 bg-teal-700 text-white rounded-lg px-5 py-4 font-bold focus:ring-2 focus:ring-teal-400 focus:outline-none"
      >
    ...
    <div class="p-5 rounded-lg shadow-lg">
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 text-center">
        ...
        <div
          ...
          :key="report.ReportID" data-aos="fade-up"
          class="max-w-4xl mx-auto bg-white border rounded-lg shadow-lg shadow-black overflow-hidden relative group transform transition-transform duration-300 hover:scale-105"
        >
          <!-- Pet Image -->
          ...
          <div class="p-4 font-bold">
            <h2 class="text-xl font-extrabold text-teal-800 mb-2">
              <span :class="report.ReportType === 'Lost' ? 'text-red-500' : 'text-teal-600'" class="m-1 text-md text-red-700">( {{ report.ReportType }} )</span>
            </h2>
            <h2 class="font-extrabold text-gray-800 mb-2">
              Reporters Name : <span class="rounded-lg p-1 text-black font-bold underline">{{ report.reporterFullName }}</span>
            </h2>
...
export default {
  data() {
    return {
      reports: [], // All reports
      filteredReports: [], // Filtered reports based on ReportType
      selectedReportType: '', // Current filter value
      currentPage: 1,
      totalPages: 0,
    };
  },
  ...
  methods: {
    fetchReports() {
      ...
      instance
        .get(`/api/reports?page=${this.currentPage}&pageSize=10`, config)
        .then((response) => {
          this.reports = response.data.reports;
          this.filteredReports = this.reports; // Default to all reports
    ...
    filterReports() {
      if (this.selectedReportType) {
        ...
        this.filteredReports = this.reports; // Reset to all reports
      }
    },
  },
};
